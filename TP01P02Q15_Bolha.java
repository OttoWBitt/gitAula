...
class Lista{
	...
	public void bolha() throws Exception{
		for(int i = n-1; i>0; i--){
			for(int j = 0; j < i; j++){
				quantidadeComparacoes += 3;

				if(array[j].getLivro() > array[j+1].getLivro()){
					swap(j, j+1);
					quantidadeMovimentacoes += 1;
					quantidadeComparacoes += 1;
				}
				else if(array[j].getLivro() == array[j+1].getLivro()){
					if(array[j].getSigla().compareTo(array[j+1].getSigla()) > 0){
						swap(j, j+1);
						quantidadeMovimentacoes += 1;	
					}
...
class TP01P02Q15_Bolha{
	...
	public static void main(String[]args) throws Exception{
		...
		long fim = now();
		Arq.openWrite("481699_bolha.txt");
