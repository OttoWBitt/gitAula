...
class Lista{
	...
	public int getMaior(){
		int maior = array[0].getPeriodico();

		for(int i = 0; i < n; i++){
			if(maior < array[i].getPeriodico()){
				quantidadeComparacoes += 1;
				maior = array[i].getPeriodico();
			}
		}
		return maior;
	}
	...
	public void countingsort() throws Exception{
		//Conta a quantidade de ocorrencias de cada elemento
		int [] counting = new int[getMaior() + 1];
		Instituicao [] ordenada = new Instituicao[n];

		//Inicializa cada posicao do array de contagem
		for(int i = 0; i < counting.length; counting[i] = 0, i++);

		//Count[i] contem o numero de elementos iguais a i
		for(int i = 0; i < n; counting[array[i].getPeriodico()]++, i++);

		//Count[i] contem o numero de elementos menores ou iguais a i
		for(int i = 1; i < counting.length; counting[i] += counting[i-1], i++);

		//Array ordenado
		for(int i = n-1; i >= 0; ordenada[counting[array[i].getPeriodico()] - 1] = array[i], counting[array[i].getPeriodico()] --, i--);

		//Criando uma copia para o array original
		for(int i = 0; i < n; array[i] = ordenada[i], i++);

		ordenacaoSelecao(0, 0, 0);
	}

	void ordenacaoSelecao(int i, int menor, int maior){
		if(i < (n-1)){
			maior = i;
			for(int j = maior + 1; j < n; j++){
				quantidadeComparacoes += 2;
				if(array[maior].getPeriodico() == array[j].getPeriodico()){
					if(array[maior].getSigla().compareTo(array[j].getSigla()) > 0){
						menor = j;
						swap(menor, maior);
			...
			ordenacaoSelecao(i+1, menor, maior);
		}
	}
}//Fim class Lista

class TP01P02Q14_CoutingSort{
	...
	public static void main(String[]args) throws Exception{
		...
		long fim = now();
		Arq.openWrite("481699_counting.txt");
